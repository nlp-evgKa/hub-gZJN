...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.linear2(x)
        x = torch.relu(x)
        x = self.linear3(x)  # (batch_size, input_size) -> (batch_size, 1)
        if y is not None:
...
def build_sample():
    ...
    # 看第一个数在5个数里排第几小
    rank = sorted(x).index(x[0])   # 0,1,2,3,4 共5个类别
    return x, rank
...
def evaluate(model):
    ...
    with torch.no_grad():
        y_pred = model(x)
        predicted = torch.argmax(y_pred, dim=1)  # 取最大值的下标作为预测类别
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():
        ...
        predicted = torch.argmax(result, dim=1)  # 取最大分数的下标
    for vec, pred in zip(input_vec, predicted):
        print("输入：%s, 预测类别：%d" % (vec, pred))
