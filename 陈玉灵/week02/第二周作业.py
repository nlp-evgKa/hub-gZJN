...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            return self.loss(x, y)  # 预测值和真实值计算损失，直接传入logits
        else:
...
def predict(model_path, input_vec):
    ...
    for vec, res, p in zip(input_vec, result, prob):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, int(torch.argmax(res).item()), p.max().item()))  # 打印结果
