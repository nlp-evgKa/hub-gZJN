...
规律：完成一个多分类任务的训练:一个随机向量，哪一维数字最大就属于第几类
...
class TorchModel(nn.Module):
    def __init__(self, input_size,class_num):
        ...
        self.activation = torch.softmax  # nn.softmax() sigmoid归一化函数
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            return self.loss(x, y)  # loss包含softmax函数，无需再调用activation函数
        else:
            return self.activation(x,dim =-1) #y_pred  # 输出预测结果
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        max_prob = res[pred_label].item()
        print("输入：%s, 预测类别：%d, 最大概率：%f" % (vec, pred_label, max_prob))  # 打印结果
